// lib/chartHelpers.js
// Plugins y tokens de estilo compartidos por todas las gráficas de Chart.js.
import { GREEN, RED } from "./colors.js";

export { GREEN, RED };

export function semanticColor(isUp) {
  return isUp ? GREEN : RED;
}

// El canvas NO resuelve variables CSS: `ctx.font = "10px var(--font-mono)"` es
// inválido, el navegador lo ignora y se queda en "10px sans-serif". Así estuvieron
// ejes y tooltips "mono" hasta el 2026-09-15. Se resuelve la variable a su
// familia real (Geist Mono vía next/font) en el momento de dibujar.
function cssFontFamily(varName, fallback) {
  if (typeof document === "undefined") return fallback;
  return getComputedStyle(document.documentElement).getPropertyValue(varName).trim() || fallback;
}

export const monoFont = (size, weight) => ({
  family: cssFontFamily("--font-mono", "ui-monospace, monospace"),
  size,
  ...(weight ? { weight } : {}),
});

export function reducedMotion() {
  return typeof matchMedia !== "undefined" && matchMedia("(prefers-reduced-motion: reduce)").matches;
}

// Único punto de entrada a Chart.js: carga perezosa + tipografía del sitio como
// default (Chart.js trae Helvetica) + sin animación si se pide reduced-motion.
export async function loadChart() {
  const { default: Chart } = await import("chart.js/auto");
  Chart.defaults.font.family = cssFontFamily("--font-sans", "system-ui, sans-serif");
  if (reducedMotion()) Chart.defaults.animation = false;
  return Chart;
}

// Glow bloom: applies canvas shadow to a specific dataset before it draws
export function makeGlowPlugin(color, datasetIndex = 1, blur = 14) {
  return {
    id: `lineGlow_${datasetIndex}`,
    beforeDatasetDraw(chart, args) {
      if (args.index !== datasetIndex) return;
      chart.ctx.shadowColor = color;
      chart.ctx.shadowBlur  = blur;
    },
    afterDatasetDraw(chart, args) {
      if (args.index !== datasetIndex) return;
      chart.ctx.shadowColor = "transparent";
      chart.ctx.shadowBlur  = 0;
    },
  };
}

// Glow bloom that reads color from each dataset's borderColor (for multi-line charts)
// datasetIndex 0 is always the fill-only dataset and should be skipped
export const multiGlowPlugin = {
  id: "multiGlow",
  beforeDatasetDraw(chart, args) {
    if (args.index === 0) return;
    const ds    = chart.data.datasets[args.index];
    const color = ds?.borderColor;
    if (!color || color === "transparent") return;
    const isLast = args.index === chart.data.datasets.length - 1;
    chart.ctx.shadowColor = color;
    chart.ctx.shadowBlur  = isLast ? 14 : 5;
  },
  afterDatasetDraw(chart, args) {
    if (args.index === 0) return;
    chart.ctx.shadowColor = "transparent";
    chart.ctx.shadowBlur  = 0;
  },
};

// Glowing terminal dot at the last point of a dataset
export function makeTerminalDotPlugin(color, datasetIndex = 1) {
  return {
    id: "terminalDot",
    afterDatasetsDraw(chart) {
      const meta = chart.getDatasetMeta(datasetIndex);
      const pts  = meta?.data;
      if (!pts?.length) return;
      const tip = pts[pts.length - 1];
      const { ctx } = chart;
      ctx.save();
      ctx.shadowColor = color;
      ctx.shadowBlur  = 18;
      ctx.beginPath();
      ctx.arc(tip.x, tip.y, 5, 0, Math.PI * 2);
      ctx.fillStyle = color;
      ctx.fill();
      ctx.shadowBlur = 0;
      ctx.beginPath();
      ctx.arc(tip.x, tip.y, 2.5, 0, Math.PI * 2);
      ctx.fillStyle = "#FFFFFF";
      ctx.fill();
      ctx.restore();
    },
  };
}

// Terminal dot that always targets the last dataset (useful for portfolio charts)
export const lastDatasetDotPlugin = {
  id: "terminalDot",
  afterDatasetsDraw(chart) {
    const lastIdx = chart.data.datasets.length - 1;
    const meta    = chart.getDatasetMeta(lastIdx);
    const pts     = meta?.data;
    if (!pts?.length) return;
    const tip   = pts[pts.length - 1];
    const color = chart.data.datasets[lastIdx]?.borderColor ?? GREEN;
    const { ctx } = chart;
    ctx.save();
    ctx.shadowColor = color;
    ctx.shadowBlur  = 18;
    ctx.beginPath();
    ctx.arc(tip.x, tip.y, 5, 0, Math.PI * 2);
    ctx.fillStyle = color;
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.beginPath();
    ctx.arc(tip.x, tip.y, 2.5, 0, Math.PI * 2);
    ctx.fillStyle = "#FFFFFF";
    ctx.fill();
    ctx.restore();
  },
};

// Crosshair vertical punteado al hover. Se desvanece hacia los extremos (patrón
// de bklit-ui, MIT) para que no corte en seco contra el borde de la card.
export const crosshairPlugin = {
  id: "crosshair",
  afterDraw(chart) {
    if (!chart.tooltip?._active?.length) return;
    const { ctx } = chart;
    const x = chart.tooltip._active[0].element.x;
    const { top, bottom } = chart.scales.y;
    const fade = ctx.createLinearGradient(0, top, 0, bottom);
    fade.addColorStop(0,    "rgba(255,255,255,0)");
    fade.addColorStop(0.15, "rgba(255,255,255,0.16)");
    fade.addColorStop(0.85, "rgba(255,255,255,0.16)");
    fade.addColorStop(1,    "rgba(255,255,255,0)");
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(x, top); ctx.lineTo(x, bottom);
    ctx.lineWidth   = 1;
    ctx.strokeStyle = fade;
    ctx.setLineDash([4, 5]);
    ctx.stroke();
    ctx.restore();
  },
};

// Revelado izquierda→derecha por recorte (clip) y desvanecido opcional del borde
// izquierdo, adaptados de bklit-ui (chart-reveal-clip.tsx, MIT (c) 2026 uixmat).
// Sustituye la animación por defecto de Chart.js, que hacía "crecer" la línea
// desde el piso del eje. Va AL FINAL del arreglo de plugins: su afterDatasetsDraw
// debe correr después del punto terminal para que también quede recortado.
// `fadeLeft` es fracción del ancho; el borde conserva 20% de opacidad porque el
// primer punto es un dato, no decoración.
export function makeRevealPlugin({ duration = 700, fadeLeft = 0 } = {}) {
  let start = null;
  let done = reducedMotion();
  const ease = (t) => (t < 0.5 ? 8 * t ** 4 : 1 - 8 * (1 - t) ** 4);
  return {
    id: "reveal",
    beforeDatasetsDraw(chart) {
      const { ctx, chartArea: a } = chart;
      let p = 1;
      if (!done) {
        const now = performance.now();
        start ??= now;
        const t = Math.min(1, (now - start) / duration);
        p = ease(t);
        if (t < 1) requestAnimationFrame(() => chart.ctx && chart.draw());
        else done = true;
      }
      ctx.save();
      ctx.beginPath();
      // Holgura de 12px a los lados para no cortar el halo del punto terminal.
      ctx.rect(a.left - 12, 0, (a.width + 24) * p, chart.height);
      ctx.clip();
    },
    afterDatasetsDraw(chart) {
      const { ctx, chartArea: a } = chart;
      if (fadeLeft > 0) {
        const w = a.width * fadeLeft;
        const g = ctx.createLinearGradient(a.left, 0, a.left + w, 0);
        g.addColorStop(0, "rgba(0,0,0,0.8)");
        g.addColorStop(1, "rgba(0,0,0,0)");
        ctx.globalCompositeOperation = "destination-out";
        ctx.fillStyle = g;
        ctx.fillRect(a.left - 12, a.top - 12, w + 12, a.height + 24);
      }
      ctx.restore();
    },
  };
}

// Vertical gradient fill factory (for backgroundColor callbacks)
export function makeGradientFn(color) {
  return (context) => {
    const area = context.chart.chartArea;
    if (!area) return "transparent";
    const g = context.chart.ctx.createLinearGradient(0, area.top, 0, area.bottom);
    g.addColorStop(0,   color + "38");
    g.addColorStop(0.6, color + "0C");
    g.addColorStop(1,   color + "00");
    return g;
  };
}

// Shared scale defaults
export const xScaleDefaults = (maxTicksLimit = 7) => ({
  ticks:  { color: "#8A8A8E", font: { size: 10 }, maxTicksLimit, maxRotation: 0 },
  grid:   { display: false },
  border: { display: false },
});

export const yScaleDefaults = (callback) => ({
  ticks:  { color: "#8A8A8E", font: monoFont(10), callback, maxTicksLimit: 5 },
  grid:   { color: "rgba(255,255,255,0.025)" },
  border: { display: false },
});

// Shared tooltip defaults
export const tooltipDefaults = {
  backgroundColor:  "rgba(10,10,12,0.92)",
  borderColor:      "rgba(255,255,255,0.08)",
  borderWidth:      1,
  cornerRadius:     10,
  titleColor:       "#6B7280",
  bodyColor:        "#F5F5F2",
  titleFont:        { size: 11 },
  padding:          12,
};

// Animación "draw-on": la línea se dibuja de izquierda a derecha punto por
// punto (patrón oficial de Chart.js para progressive line). Devuelve el objeto
// `animation` para options; pasar `false` si el usuario pide reduced-motion.
export function progressiveLine(pointCount, totalMs = 900) {
  const per = totalMs / Math.max(pointCount, 1);
  const delay = (ctx) => {
    if (ctx.type !== "data" || ctx.mode !== "default" || ctx.dropped) return 0;
    ctx.dropped = true;
    return ctx.index * per;
  };
  return {
    x: { type: "number", easing: "linear", duration: per, from: NaN, delay },
    y: { type: "number", easing: "linear", duration: per, from: "previousY", delay },
  };
}

// Card container style
export function cardStyle(isUp = null) {
  return {
    background:           "rgba(4,4,5,0.80)",
    backdropFilter:       "blur(20px)",
    WebkitBackdropFilter: "blur(20px)",
    border:               isUp === null
      ? "1px solid rgba(255,255,255,0.07)"
      : `1px solid ${isUp ? GREEN : RED}26`,
    borderRadius:         20,
  };
}

// Section label style
export const sectionLabel = {
  fontSize: 11, letterSpacing: 2.5, textTransform: "uppercase",
  color: "#8A8A8E", fontFamily: "var(--font-mono)",
};
