...
export function buildDailyBundle(closes, highs, lows, extra = {}) {
  ...
  return {
    ...
    eventMultiplier: extra.eventMultiplier ?? 1, // ya escalado por proximidad (lib/events.js)
  };
}
...
export function computeConviction(b, dir) {
  ...
  // Multiplicador por evento (Fed/CPI/Banxico) HOY o PRÓXIMO → ya viene escalado
  // por cercanía desde lib/events.js (el chart es poco fiable cerca de eventos).
  const evMult = b.eventMultiplier ?? 1;
...
export function computeTAIndex(bundle, opts = {}) {
  ...
  const b = { ...bundle, eventMultiplier: opts.eventMultiplier ?? bundle.eventMultiplier ?? 1 };
...
export const TA_BANDS = [
  { max: 20, key: "OVERSOLD",     es: "Muy estirado a la baja", en: "Deeply oversold",  color: GREEN },
  ...
  { max: 100, key: "OVERBOUGHT",  es: "Muy estirado al alza",   en: "Deeply overbought", color: RED },
];
