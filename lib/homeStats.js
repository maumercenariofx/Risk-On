...
//
// Devuelve { n, atLeast } (2026-09-11). Si la racha llega hasta el primer view
// disponible (n === postsDesc.length), lo que se midió es el inicio del
// ARCHIVO, no el del régimen: n es cota inferior y la UI lo pinta "N+".
export function regimeAge(postsDesc) {
  if (!postsDesc?.length) return { n: 0, atLeast: false };
  const head = bandOf(postsDesc[0]);
  if (!head) return { n: 0, atLeast: false };
  let n = 0;
  for (const p of postsDesc) {
    if (bandOf(p) !== head) break;
    n++;
  }
  return { n, atLeast: n === postsDesc.length };
}
