...
// ── Normalización dinámica (anti-clipping) ───────────────────────────────────
// Cada señal con historia disponible se normaliza con un z-score ROBUSTO y se
// mapea con una logística suave, en vez de la rampa fija con tope duro:
//   • Centro = ancla ABSOLUTA (0 para retornos, un normal de largo plazo para
//     niveles) → preserva el significado risk-on/off del número.
//   • Escala = MAD rodante de ~60 días (desviación robusta) → se ensancha en
//     pánico y se encoge en calma, así el rango se auto-ajusta al régimen.
//   • Logística → las colas nunca se clavan en 0/100; un +3% sigue leyendo más
//     extremo que un +1.5%. Calibrado con backtests de 5 años.
// Si no hay historia suficiente (p.ej. datos de respaldo), cae a la rampa fija
// lin() → el número en vivo nunca se rompe. Las señales lentas/estructurales
// (carry, curva) NO se tocan: siguen en rampa fija, es lo correcto para ellas.
// PISO DE ESCALA para señales de NIVEL (calibración 2026-07-13): la MAD corta
// mide la estabilidad reciente, no la distancia plausible al ancla — en calma
// lejos del ancla el z explotaba y la logística saturaba (MOVE clavado en sub
// 100 desde el 25-jun: z=-9.2 con MAD60=3.3). escala = max(MAD60, piso), donde
// el piso = MAD de 5 años del nivel (scripts/calibrate-scale-floor.mjs). El MAD
// rodante sigue mandando en pánico (crece por encima del piso). Saturación en
// backtest 5y: MOVE 31.7%→0.2%, mxnvol 18.2%→2.4%, VIX 19%→3.9% de los días.
// Las señales de RETORNO no llevan piso: su MAD corta es la escala correcta.
const DYN_N = 60, DYN_K = 1.1, DYN_MIN = 30;
const DYN = {
  vix:    { anchor: 17.5, sign: -1, minScale: 4.4,  series: (d) => d.market?.vixSeries },
  mxn:    { anchor: 0,    sign: -1, series: (d) => d.market?.usdmxnChgSeries },
  spx:    { anchor: 0,    sign:  1, series: (d) => d.market?.spxChgSeries },
  mxnvol: { anchor: 9,    sign: -1, minScale: 4.1,  series: (d) => d.market?.mxnVolSeries },
  move:   { anchor: 100,  sign: -1, minScale: 28,   series: (d) => d.market?.moveSeries },
  btc:    { anchor: 0,    sign:  1, series: (d) => d.market?.btcChgSeries },
  gold:   { anchor: 0,    sign: -1, series: (d) => d.market?.goldChgSeries },
};
...
// Desviación absoluta mediana escalada a σ (1.4826 = consistencia gaussiana).
function madScale(w) {
  const med = median(w);
  return 1.4826 * median(w.map((v) => Math.abs(v - med)));
}
// Sub-score dinámico (0-100) o null si no hay historia suficiente → fallback.
function dynSub(value, series, anchor, sign, minScale = 0) {
  if (value == null || isNaN(value) || !Array.isArray(series)) return null;
  const w = series.filter((x) => x != null && !isNaN(x)).slice(-DYN_N);
  if (w.length < DYN_MIN) return null;
  const scale = Math.max(madScale(w), minScale);
  if (!scale || !isFinite(scale)) return null;
  const z = (value - anchor) / scale;
  return 100 / (1 + Math.exp(-DYN_K * sign * z));
}
...
export function computeRiskScore(d) {
  ...
  for (const s of SIGNALS) {
    const raw = s.get(d);
    const cfg = DYN[s.key];
    let sub = cfg ? dynSub(raw, cfg.series(d), cfg.anchor, cfg.sign, cfg.minScale) : null;
    if (sub == null) sub = lin(raw, s.at0, s.at100); // fallback rampa fija
...
// Cortes recalibrados (quantile-matching sobre backtest de 5 años) para que la
// distribución por banda preserve la proporción histórica. Actualizados
// 2026-07-13 con el piso de escala (antes 29/48/72): RISK-OFF ~2.8% de los
// días, RISK-ON ~3.6% — bandas de cola, reservadas para extremos reales.
export const BANDS = [
  { max: 32,  key: "RISK-OFF",     es: "Risk-off",     en: "Risk-off",     color: "#5B7FB9" },
  { max: 49,  key: "DEFENSIVE",    es: "Defensivo",    en: "Defensive",    color: "#D9A227" },
  { max: 67,  key: "CONSTRUCTIVE", es: "Constructivo", en: "Constructive", color: "#2FB89A" },
