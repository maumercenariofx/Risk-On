...
const DYN = {
  vix:    { anchor: 17.5, sign: -1, series: (d) => d.market?.vixSeries },
  mxn:    { anchor: 0,    sign: -1, series: (d) => d.market?.usdmxnChgSeries },
  spx:    { anchor: 0,    sign:  1, series: (d) => d.market?.spxChgSeries },
  mxnvol: { anchor: 9,    sign: -1, series: (d) => d.market?.mxnVolSeries },
  move:   { anchor: 100,  sign: -1, series: (d) => d.market?.moveSeries },
...
function dynSub(value, series, anchor, sign) {
...
export function computeRiskScore(d) {
  ...
  for (const s of SIGNALS) {
    const raw = s.get(d);
    const cfg = DYN[s.key];
    let sub = cfg ? dynSub(raw, cfg.series(d), cfg.anchor, cfg.sign) : null;
...
// distribución por banda del método dinámico preserve la proporción histórica.
export const BANDS = [
  { max: 29,  key: "RISK-OFF",     es: "Risk-off",     en: "Risk-off",     color: "#5B7FB9" },
  { max: 48,  key: "DEFENSIVE",    es: "Defensivo",    en: "Defensive",    color: "#D9A227" },
  { max: 72,  key: "CONSTRUCTIVE", es: "Constructivo", en: "Constructive", color: "#2FB89A" },
