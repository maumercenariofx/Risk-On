...
// Días inhábiles de los mercados mexicanos (BMV/Banxico) 2026. Sirven para
// detectar puentes y decir "nos vemos el martes" en vez de "mañana".
const MX_HOLIDAYS = new Set([
  ...
  // 2027 (por si el cron corre en enero antes de actualizar)
  "2027-01-01",
]);
...
function isTradingDay(d) {
  ...
  return dow !== 0 && dow !== 6 && !MX_HOLIDAYS.has(ymd(d));
}
// Próxima sesión hábil después de `slug` (YYYY-MM-DD).
function nextTradingDay(slug) {
...
function dayGreetings(slug) {
  ...
  const next = nextTradingDay(slug);
  // brecha en días naturales hasta la próxima sesión
  const gapDays = Math.round((next - today) / 86400000);
  const nextName = DOW[next.getUTCDay()];
  const nextNameEn = DOW_EN[next.getUTCDay()];
  const isPuente = gapDays > 3 || (dow !== 5 && gapDays > 1); // fin de semana largo
  ...
  // Despedida según lo que sigue
  let signoff_es, signoff_en;
  if (isPuente) {
    signoff_es = `Disfruta el fin de semana largo — nos vemos el ${nextName} con otro view. ☕`;
    signoff_en = `Enjoy the long weekend — back on ${nextNameEn} with another view. ☕`;
  } else if (dow === 5) {
