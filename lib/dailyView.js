...
function userPrompt(digest, dateStr, score, prev, todaySlug) {
  ...
  for (let attempt = 1; attempt <= 3 && !view; attempt++) {
    try {
      const req = {
        ...
        system: SYSTEM,
        messages: [{ role: "user", content: userPrompt(digest, dateStr, scoreInfo.score, prev, slug ?? new Date().toISOString().slice(0, 10)) }],
      };
      if (structured) {
