...
function userPrompt(digest, dateStr, score, prev) {
  // Bloque de memoria: el view de ayer, SOLO como referencia de continuidad y
  ...
  const prevBlock = prev
    ? `
VIEW DE AYER (${prev.slug}) — referencia de continuidad; PROHIBIDO copiar sus formulaciones:
- Titular: ${prev.title}
- Score de ayer: ${prev.score}/100
- Subtítulos que usó: ${prev.headings.join(" · ") || "(s/d)"}
- Postura de ayer: ${prev.postura || "(sin postura explícita)"}
- Resumen: ${prev.summary}

CONTINUIDAD Y ANTI-REPETICIÓN:
- El lector diario debe sentir un HILO, no un reinicio: cuando aporte, di qué pasó con lo que ayer se señaló ("ayer marcamos la resistencia en X — hoy el par la respetó/rompió", "el score pasa de ${prev.score} a ${score} porque...").
- PROHIBIDO reutilizar los subtítulos de ayer o variaciones obvias, el mismo gancho de titular, o sus frases características (revisa la postura de ayer y formúlala distinto si el sesgo no cambió — o mejor: explica POR QUÉ sigue vigente con el dato de hoy).
`
    : "";
...
const VIEW_SCHEMA = {
  ...
  required: ["title_es", "title_en", "summary_es", "summary_en", "support",
    "resistance", "watch_es", "watch_en", "body_es", "body_en"],
...
export async function generateDailyView(data, dateStr, slug) {
  ...
  // Memoria del día hábil anterior (best-effort: null si no se encuentra).
  const prev = await fetchPrevView(slug ?? new Date().toISOString().slice(0, 10)).catch(() => null);
  ...
  for (let attempt = 1; attempt <= 3 && !view; attempt++) {
    try {
      const req = {
        ...
        max_tokens: 6000,
        system: SYSTEM,
        messages: [{ role: "user", content: userPrompt(digest, dateStr, scoreInfo.score, prev) }],
      };
      if (structured) {
