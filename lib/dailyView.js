...
function dataDigest(data, scoreInfo, notables, extras = {}) {
  ...
  if (Array.isArray(cal) && cal.length) {
    ...
    if (hi.length) lines.push(`CALENDARIO (próx. eventos alto impacto, hora CDMX): ${hi.map((e) => `${e.date}${e.time ? ` ${e.time}` : ""} ${e.flag} ${e.event_es}`).join(" · ")}`);
  }
...
function loadEditorialGuide() {
  try {
    ...
    // §7 (registro de errores) queda FUERA del bloque delimitado a propósito:
    // es append-only y crece sin techo. Se extrae aparte y se inyectan solo
    // las últimas 25 entradas.
    //
    // Sin esto, los 19 errores REALES que la auditoría del corpus documentó
    // —cada uno con su cita textual— estaban en el archivo y NO llegaban al
    // redactor: el bloque delimitado termina en §6. Verificado el 2026-08-24:
    // 0 de 19 se estaban inyectando. Documentar un error sin ponérselo
    // enfrente a quien lo cometió no previene nada.
    const errores = raw
      .split(/\r?\n/)
      .filter((l) => /^\s*[-*]\s+`\[\d{4}-\d{2}-\d{2}\]`/.test(l))
      .map((l) => l.replace(/^\s*[-*]\s+/, "- ").replace(/`/g, "").replace(/\s+/g, " ").trim())
      .slice(-25);
    const bloqueErrores = errores.length
      ? `\n\nERRORES QUE YA COMETISTE (salieron publicados; no los repitas):\n${errores.join("\n")}`
      : "";
    return `\n\nMANUAL EDITORIAL DE LA CASA (docs/references/views-editorial.md — reglas estables, mandan sobre cualquier costumbre):\n${core.trim()}${bloqueErrores}`;
  } catch (e) {
...
Devuelve SOLO un objeto JSON válido (sin texto antes ni después, sin bloque de código markdown) con esta forma EXACTA:
{
  ...
  "body_es": "cuerpo en Markdown, español, ENTRE 250 Y 700 palabras: la longitud es una decisión editorial, no una cuota. Un día sin nada que decir merece 280 palabras y decirlo; un día de FOMC merece 650. ENTRE 2 Y 5 subtítulos '### ...', tú decides cuántos según lo que el día merezca, y el ORDEN lo manda la importancia de HOY, no una plantilla. Los subtítulos nacen del ÁNGULO específico de hoy — nunca genéricos reutilizables tipo 'La señal del día' o 'Tasas y curva', ni parecidos a los de ayer. PROHIBIDO abrir la segunda sección con 'El peso' (lo hiciste en 44 de 58 views). Incluye UNA postura direccional FALSABLE: sesgo + nivel de referencia + qué la invalidaría, enmarcada como opinión de mercado, sin recomendaciones operativas — y CAMBIA LA SINTAXIS cada día: revisa cómo formulaste la del view anterior y usa otra construcción, porque 'el sesgo favorece al peso mientras...' ya apareció 24 veces. Jerarquía de cifras: el cuerpo explica el PORQUÉ — usa los números para sostener argumentos, cada cifra UNA sola vez, sin re-listar el digest completo. El Risk On score ${score} (${riskState(score)}) va UNA vez, donde sostenga el argumento, y NUNCA como cierre: el correo y el sitio ya lo muestran arriba, y terminar con él se volvió plantilla (14 de 15 views cerraban con 'el Risk On sube/baja/queda...'). Termina con lo que el lector se lleva a su mesa (la condición, el siguiente juez o el nivel a vigilar) en una oración de menos de 20 palabras. Si tu postura y su condición son las MISMAS del view anterior, confírmalas UNA sola vez en todo el artículo, con la cifra (esa frase puede ser el cierre), en vez de desarrollarlas otra vez: del 24 al 28-ago la misma condición salió palabra por palabra cinco días seguidos.",
  "body_en": "the same content written NATIVELY in English — NOT a translation. Same numbers and the same falsifiable view, but decide the paragraph breaks and the emphasis AGAIN for this reader: a desk reader in New York or London who follows LatAm FX. Cut what a US reader does not need, add the context they do. Do NOT mirror the Spanish sentence by sentence — the giveaway is that both versions end up with identical bolding and identical paragraph counts. Em dash (—) twice at most in the whole piece. The construction 'X, not Y' is FORBIDDEN. Different emphasis does NOT mean none: body_en also bolds 2-3 key figures per section, because this reader scans too (on 2026-09-07 the English version shipped with zero bold against twelve in Spanish). The Risk On score appears once where it supports the argument, never as the closing line."
}`;
}
  ...
  for (let i = 0; i < 20 && out.length < want; i++) {
    ...
    try {
      ...
      out.push({
        ...
        // ¿Citó el posicionamiento de Chicago? Alimenta cotParaLente().
        cot: COT_CITADO.test(content),
      });
    } catch {}
  }
  return out;
...
// ── Anti-monotonía del lente COT (2026-09-11) ───────────────────────────────
// En los 15 views del 21-ago al 10-sep "netos largos" salió en 15 y "los
// especuladores en Chicago" en 12: el reporte es SEMANAL y el redactor lo
// reciclaba a diario, porque el digest se lo ofrecía a diario con un "úsalo
// máximo una vez" que el modelo no puede cumplir sin ver sus textos
// anteriores. Mismo remedio que el veto temático: se decide en código.
// La CFTC publica el viernes 15:30 ET las posiciones al martes, así que el
// primer view que ve un reporte es el del día hábil siguiente a ese viernes.
// Entra al lente si el reporte es nuevo para el lector, o si ninguno de los
// últimos 3 views lo citó.
const COT_CITADO = /Chicago|\bCOT\b|especuladores|netos (?:largos|cortos)/i;
export function cotParaLente(cot, prevs) {
  if (!cot?.date) return { cot: null, motivo: "sin dato" };
  const viernes = new Date(`${cot.date}T12:00:00Z`);
  viernes.setUTCDate(viernes.getUTCDate() + 3);
  const prevSlug = prevs?.[0]?.slug;
  if (!prevSlug || prevSlug <= ymd(viernes)) return { cot, motivo: "reporte nuevo para el lector" };
  const citado = (prevs ?? []).slice(0, 3).filter((p) => p.cot).length;
  if (citado === 0) return { cot, motivo: "ningún view de los últimos 3 lo citó" };
  return { cot: null, motivo: `fuera del lente: ya salió en ${citado} de los últimos 3 views y no hay reporte nuevo desde el ${cot.date}` };
}
...
export async function generateDailyView(data, dateStr, slug) {
  ...
  const cotSel = cotParaLente(cot, prevs);
  const digest = dataDigest(data, scoreInfo, notables, { angles, cot: cotSel.cot });
  ...
  if (cot) console.log(`[gen] COT MXN al ${cot.date}: net ${cot.net} (Δ ${cot.dNet}) — ${cotSel.motivo}`);
  if (prior) console.log(`[gen] prior de postura: ${prior.bias ?? "sin dirección"} ${prior.fuerza} (stretch ${prior.stretch?.toFixed?.(2) ?? "s/d"}) — ${prior.señal}`);
  ...
  for (const aviso of avisosSuaves(view)) console.log(`[gen] aviso (no bloquea el envío): ${aviso}`);
  ...
  if (prior) {
    // Desde el 2026-08-28 el prior puede NO tener dirección: bias null significa
    // "el modelo no opina hoy", que es distinto de "el modelo dice neutral".
    // Solo se escribe el campo cuando hay una lectura real, para que la marca
    // "≠ prior" del marcador no compare contra una opinión inventada.
    if (prior.bias) view.prior_bias = prior.bias;
...
// Avisos que NO disparan reintento (2026-09-11): cada reintento del redactor
// cuesta ~150 s y el correo sale con ~5 min de margen antes de las 7:00, así
// que el estilo se corrige en el prompt y aquí solo se MIDE si se cumple.
export function avisosSuaves(v) {
  const out = [];
  const negritas = (t) => ((t ?? "").match(/\*\*[^*\n]+\*\*/g) || []).length;
  const es = negritas(v.body_es), en = negritas(v.body_en);
  if (es >= 3 && en === 0) out.push(`body_en salió sin negritas (body_es trae ${es}).`);
  const ultimo = (v.body_es ?? "").trim().split(/\n+/).filter((l) => l.trim()).pop() ?? "";
  if (/risk on|\bscore\b/i.test(ultimo)) out.push("el último párrafo de body_es volvió a cerrar con el score.");
  // La antítesis «X, no Y» está prohibida en el prompt y el dry-run del
  // 11-sep la usó dos veces; aquí solo se cuenta (aproximado, sin reintento).
  const antitesis = ((v.body_es ?? "").match(/, no (?:el|la|los|las|un|una|es|son|está|están|por|de|del|al|en|a) /gi) || []).length;
  if (antitesis) out.push(`antítesis «X, no Y» ×${antitesis} en body_es (el prompt la prohíbe).`);
  return out;
}
