...
function userPrompt(digest, dateStr, score, prev, todaySlug, pulse, olderPrevs, record, vetoes) {
...
` : ""}${pulseBlock}${vetoBlock}${prevBlock}${weekBlock}${recordBlock}
...
export async function generateDailyView(data, dateStr, slug) {
  ...
  for (let attempt = 1; attempt <= 4 && !view; attempt++) {
    ...
    try {
      const prompt =
        userPrompt(digest, dateStr, scoreInfo.score, prev, todaySlug, pulse, olderPrevs, record, vetoes) + fixNotes;
      if (useCli) {
