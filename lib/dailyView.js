...
function dataDigest(data, scoreInfo) {
  ...
  if (m) {
    ...
    if (spot != null && m.usdmxnChg != null) {
      const prevClose = spot / (1 + m.usdmxnChg / 100);
      const cents = (spot - prevClose) * 100;
      const abs = Math.abs(cents);
      if (abs >= 5) {
        const dir = cents > 0 ? "SUBE" : "BAJA";
        const peso = cents > 0 ? "el peso pierde" : "el peso gana";
        const mag = abs >= 20 ? "MOVIMIENTO FUERTE, es LA historia del día" : abs >= 10 ? "movimiento importante" : "movimiento a destacar";
        notables.push(`USD/MXN ${dir} ${abs.toFixed(0)} centavos vs el cierre previo (${fmt(prevClose, 4)} → ${fmt(spot, 4)}), ${peso} — ${mag}`);
      }
      if (m.mxnR1 != null && spot > m.mxnR1) {
        notables.push(`ROMPIÓ la resistencia de 10 días (${fmt(m.mxnR1, 4)}) — cotiza ${((spot - m.mxnR1) * 100).toFixed(0)} centavos arriba`);
      } else if (m.mxnR1 != null && (m.mxnR1 - spot) * 100 <= 5) {
        notables.push(`A ${((m.mxnR1 - spot) * 100).toFixed(0)} centavos de la resistencia de 10 días (${fmt(m.mxnR1, 4)})`);
      }
      if (m.mxnS1 != null && spot < m.mxnS1) {
        notables.push(`PERFORÓ el soporte de 10 días (${fmt(m.mxnS1, 4)}) — cotiza ${((m.mxnS1 - spot) * 100).toFixed(0)} centavos abajo`);
      } else if (m.mxnS1 != null && (spot - m.mxnS1) * 100 <= 5) {
        notables.push(`A ${((spot - m.mxnS1) * 100).toFixed(0)} centavos del soporte de 10 días (${fmt(m.mxnS1, 4)})`);
      }
    }
    if (notables.length) {
      lines.push("");
      ...
      lines.push(
        `REGLA DE MOVIMIENTOS (obligatoria): el hecho más importante de esta lista debe aparecer TEXTUALMENTE, con su cifra en CENTAVOS, en el summary y en el body ("sube 15 centavos", "a 3 centavos de la resistencia") — el porcentaje solo puede acompañar, nunca sustituir. Los traders hablan en centavos. En body_en usa "centavos" igual (15 centavos ≈ 0.15 pesos). Si el movimiento es FUERTE, considera llevarlo al titular.`
      );
    }
  }
...
export async function generateDailyView(data, dateStr, slug) {
  ...
  for (let attempt = 1; attempt <= 3 && !view; attempt++) {
    try {
      if (useCli) {
        const res = await claudeCli(
          userPrompt(digest, dateStr, scoreInfo.score, prev, slug ?? new Date().toISOString().slice(0, 10), pulse),
          { system: SYSTEM, schema: VIEW_SCHEMA, tools: "", model: "claude-opus-4-8" }
        );
        view = res.structured_output ?? JSON.parse(res.result);
        continue;
      }
      const req = {
