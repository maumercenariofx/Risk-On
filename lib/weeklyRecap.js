...
async function usdmxnWeek(monday, friday) {
  try {
    ...
    for (let i = 0; i < ts.length; i++) {
      ...
      // OJO convención Yahoo FX: la vela diaria va timestampeada a la
      // MEDIANOCHE DE LONDRES del arranque de sesión (23:00 UTC del día
      // anterior en verano) — sin el +1h cada cierre queda etiquetado con el
      // día equivocado (el bug hacía pasar el cierre del martes por "lunes").
      const d = new Date((ts[i] + 3600) * 1000).toISOString().slice(0, 10);
      if (d >= monday && d <= friday) {
        days.push({ d, c: +c.toFixed(4) });
        hi = Math.max(hi, highs[i] ?? c);
        lo = Math.min(lo, lows[i] ?? c);
      }
    }
    if (!days.length) return null;
    const chg = ((days[days.length - 1].c - days[0].c) / days[0].c) * 100;
    return {
      path: days, open: days[0].c, close: days[days.length - 1].c, chgPct: +chg.toFixed(2),
      hi: isFinite(hi) ? +hi.toFixed(4) : null, lo: isFinite(lo) ? +lo.toFixed(4) : null,
    };
  } catch {
    return null;
  }
