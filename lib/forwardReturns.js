...
export async function posturaRecord(posts) {
  try {
    ...
    return {
      // Orden EXPLÍCITO, más reciente primero. Antes era `[...rows].reverse()`,
      // que solo da "más reciente primero" si quien llama pasa los posts en
      // orden ascendente. /indice sí lo hace, pero getAllPostsMeta() los da
      // descendentes, así que el redactor diario (fetchPosturaRecord) y el
      // recap (gatherWeek) recibían las posturas MÁS VIEJAS como las últimas.
      // El log de gen decía "última 2026-07-10" el 10-sep, la regla de
      // rendición de cuentas nunca podía dispararse, y el recap del 4-sep
      // publicó que "no se resolvieron posturas nuevas, las que maduraron son
      // de julio" cuando sí maduraron las de finales de agosto (2026-09-11).
      rows: [...rows].sort((a, b) => (a.slug < b.slug ? 1 : -1)),
