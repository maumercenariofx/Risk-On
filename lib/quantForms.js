...
void main() {
  float frontVis = smoothstep(-0.06, 0.10, vFacing);
  vec3 col = mix(uFlowColor, vec3(1.0), 0.55 + vSpecial * 0.3);
  vec4 dotTex = texture2D(uDot, gl_PointCoord);
  gl_FragColor = vec4(col * 1.6, 1.0) * dotTex * frontVis * uColorT * (0.75 + vSpecial * 0.25);
}
`;
