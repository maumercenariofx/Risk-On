...
// en reversa (palíndromo), descansa, repite. Las partículas se quedan en la
...
// al globo). Devuelve el índice del movimiento activo (la banda que "engorda").
export function rubikTwistAt(t, out, k = 1) {
  const sc = solveCycle(t, RUBIK_MOVES, 0.42, 1.2);
  for (let i = 0; i < RUBIK_MOVES; i++) {
    const m = rubikMoves[i];
    out[i].set(m.axis, m.lo, m.hi, m.ang * sc.amount[i] * k);
  }
  return sc.active;
}
