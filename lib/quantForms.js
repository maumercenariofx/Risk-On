...
function buildGeoCanvas(m) {
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      land[y * W + x] = (bytes[i >> 3] & (1 << (i & 7))) ? 255 : 0;
    }
  }
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      const p = idx * 4;
      img.data[p] = v; img.data[p+1] = coast; img.data[p+2] = countryId[idx] * 51; img.data[p+3] = 255;
    }
  }
...
export const RISK_COUNTRIES = [
  { id: "mx", name_es: "México",         name_en: "Mexico",        lat: 23.6,  lon: -102.5, score: 62, phase: 0.0 },
  { id: "us", name_es: "Estados Unidos", name_en: "United States", lat: 39.8,  lon: -98.6,  score: 48, phase: 1.3 },
  { id: "cn", name_es: "China",          name_en: "China",         lat: 35.0,  lon: 103.8,  score: 70, phase: 2.6 },
  { id: "br", name_es: "Brasil",         name_en: "Brazil",        lat: -10.3, lon: -53.2,  score: 65, phase: 3.9 },
  { id: "tr", name_es: "Turquía",        name_en: "Turkey",        lat: 38.9,  lon: 35.2,   score: 88, phase: 5.2 },
];
...
void main() {
  ...
  vec3 riskGreen   = vec3(0.20, 0.42, 0.22);
  vec3 riskYellow  = vec3(0.55, 0.50, 0.20);
  vec3 riskRed     = vec3(0.64, 0.18, 0.18);

  vec3 geo = mix(oceanColor, landColor, vLand);
  geo = mix(geo, borderColor, vBorder);

  // Traffic-light highlight for the curated risk countries (vCountryId 1-5,
  // matching RISK_COUNTRIES / COUNTRY_MASK_B64 order), tinted by stress score
  // and pulsing at a rate + intensity that scales with that score, each with
  // its own phase so the countries don't blink in sync. Land-only and shaped
  // by real country polygons, so it never bleeds across borders.
  int cid = int(vCountryId + 0.5);
  if (cid >= 1 && cid <= 5) {
    vec2 cd = vec2(0.0);
    if (cid == 1) cd = uCountryData[0];
    else if (cid == 2) cd = uCountryData[1];
    else if (cid == 3) cd = uCountryData[2];
    else if (cid == 4) cd = uCountryData[3];
    else cd = uCountryData[4];
    float score = cd.x;
    vec3 riskColor = score < 0.5
      ? mix(riskGreen, riskYellow, score * 2.0)
      : mix(riskYellow, riskRed, (score - 0.5) * 2.0);
    float pulse = 0.5 + 0.5 * sin(uTime * (0.5 + score * 3.0) + cd.y);
    float amt = (0.15 + 0.85 * score) * pulse * uColorT;
  ...
  float frontVis = smoothstep(-0.08, 0.05, vFacing);
