...
uniform float uScan;
// Giro Rubik (forma RUBIK): hasta 14 movimientos de rebanada. Cada vec4 es
// (eje 0/1/2, límite inferior, límite superior, ángulo YA escalado por su
// avance). uTwistOn = 0 → sin giro, cero costo. uTwistActive = índice del
// movimiento en curso: sus puntos engordan ("la mano" que gira el cubo).
uniform vec4  uTwist[14];
uniform float uTwistOn;
uniform float uTwistR;
uniform float uTwistActive;
float twistHand;
varying vec3 vColor;
varying vec3 vDir;
varying float vLand;
...
vec3 rubikTwist(vec3 p) {
  for (int i = 0; i < 14; i++) {
    vec4 mv = uTwist[i];
    if (mv.w == 0.0) continue;
    float c = mv.x < 0.5 ? p.x : (mv.x < 1.5 ? p.y : p.z);
    if (c < mv.y || c >= mv.z) continue;
    if (abs(float(i) - uTwistActive) < 0.5) twistHand = 1.0;
    float ca = cos(mv.w), sa = sin(mv.w);
    if (mv.x < 0.5)      p = vec3(p.x, p.y * ca - p.z * sa, p.y * sa + p.z * ca);
    else if (mv.x < 1.5) p = vec3(p.x * ca + p.z * sa, p.y, -p.x * sa + p.z * ca);
    else                 p = vec3(p.x * ca - p.y * sa, p.x * sa + p.y * ca, p.z);
  }
  return p;
}
...
void main() {
  ...
  vec3 pos = position;
  // Giro Rubik: las rebanadas se cortan en coordenadas unitarias. Al pasar
  // del cubo al globo el giro se desvanece (ángulos → 0) durante el morph.
  twistHand = 0.0;
  if (uTwistOn > 0.5) pos = rubikTwist(pos / uTwistR) * uTwistR;
  ...
  // Barrido de búsqueda (orbe "searching", 2026-09-21): mientras el score no
  // llega, un meridiano recorre el globo levantando y agrandando sus puntos.
  // uScan 1 → 0 cuando RiskGauge llama setHalo. 100% shader.
  vScan = 0.0;
  if (uScan > 0.001) {
    float d = atan(dir.z, dir.x) - uTime * 1.6;
    d = atan(sin(d), cos(d));
    vScan = exp(-d * d / 0.045) * uScan * uColorT;
    pos += dir * vScan * 0.035;
  }
  ...
  // Normal de la posición girada: sin esto, durante el giro Rubik el corte
  // del hemisferio lejano se calcula con la orientación vieja.
  ...
  float size = uSize * (1.0 + vBorder * uColorT * 0.9) * (1.0 + vScan * 0.9) * (1.0 + twistHand * 0.3);
...
void main() {
  ...
  geo *= mix(1.0, uNightFloor, night * vLand);
  ...
  for (int i = 0; i < 8; i++) {
    ...
    geo += cityCol * g * max(night, 0.25 * uNightAmt) * uCityGlow[i] * tw * 2.1 * uCityBoost;
  }
  ...
  // Barrido de búsqueda: lo no barrido se atenúa para que el meridiano se lea
  // (el dimBase 0.45 del orbe original), y la banda brilla en blanco frío.
  float scanOn = uScan * uColorT;
  finalColor *= (1.0 - 0.45 * scanOn) + vScan * 1.4;
  finalColor += vec3(0.55, 0.75, 1.0) * vScan * 0.3;
...
// Cabezas de cometa: PUNTOS con glow viajando por los arcos — las líneas de
// 1px casi no se ven en DPR 3 de teléfono; estos puntos sí. La posición se
// calcula EN EL SHADER (aA→aB por fase), así que el buffer es estático.
export const COMET_VERTEX_SHADER = /* glsl */ `
precision highp float;
attribute vec3 aA;
attribute vec3 aB;
...
void main() {
  vSpecial = aSpecial;
  float speed = mix(0.16, 0.26 + uMxnPulse, aSpecial);
  float t = fract(aPhase + uTime * speed);
  float tt = uFlowDir > 0.0 ? t : 1.0 - t;
  vec3 p = normalize(mix(aA, aB, tt));
  p *= uRadius * (1.0 + sin(3.14159265 * tt) * aLift);
  vec4 mv = modelViewMatrix * vec4(p, 1.0);
  vec3 n = normalize(normalMatrix * normalize(p));
  vFacing = dot(n, normalize(-mv.xyz));
  gl_PointSize = uSize * (1.0 + aSpecial * 0.5) * uPixelsPerUnit * uPixelRatio / -mv.z;
  gl_Position = projectionMatrix * mv;
}
`;

export const COMET_FRAGMENT_SHADER = /* glsl */ `
precision highp float;
uniform vec3 uFlowColor;
uniform float uColorT;
varying float vSpecial;
varying float vFacing;
void main() {
  // Glow PROCEDURAL: núcleo caliente chiquito + halo exponencial suave.
  // (La textura de puntito de las partículas, estirada a este tamaño, se
  // veía como disco plano — "círculos feos" en el teléfono.)
  vec2 uv = gl_PointCoord - 0.5;
  float r2 = dot(uv, uv) * 4.0;          // 0 centro → 1 borde del sprite
  float core = exp(-r2 * 16.0);
  float halo = exp(-r2 * 3.0) * 0.38;
  float glow = core + halo;
  float frontVis = smoothstep(-0.06, 0.10, vFacing);
  vec3 col = mix(uFlowColor, vec3(1.0), core * 0.7 + vSpecial * 0.2);
  gl_FragColor = vec4(col * 1.5, 1.0) * glow * frontVis * uColorT * (0.8 + vSpecial * 0.2);
}
`;

export function makeCometGeometry(THREE, r) {
  const aA = [], aB = [], lifts = [], phases = [], specials = [], pos = [];
  const byId = Object.fromEntries(FIN_CENTERS.map((c) => [c.id, c]));
  const dirOf = (c) => { const d = latLonToDir(c.lat, c.lon); return [d.x, d.y, d.z]; };
  for (const [aId, bId, special] of FLOW_ROUTES) {
    ...
    const lift = 0.10 + 0.16 * (ang / Math.PI);
    const K = special ? 4 : 3; // la arteria NY↔CDMX lleva un cometa extra
    for (let k = 0; k < K; k++) {
      aA.push(...a); aB.push(...b);
      lifts.push(lift);
      phases.push(k / K + (special ? 0 : 0.13 * ((aId.charCodeAt(0) + bId.charCodeAt(0)) % 7)));
      specials.push(special);
      pos.push(a[0] * r, a[1] * r, a[2] * r); // placeholder (la posición real la da el shader)
    }
  }
...
// ── Orbes de puntos (experimento 2026-09-21) ─────────────────────────────
// Puerto del modo "rubik" (solving) de
// thinking-orbs — MIT © Jakub Antalik, github.com/RareFormLabs/thinking-orbs.
// Allá cada punto es un disco de canvas 2D (cientos de puntos); aquí cada
// punto del orbe es un CÚMULO de partículas: repartir 72k-220k partículas
// parejo sobre la figura daba un bloque sólido y se perdía el look de puntos.
...
// Retícula lat/long del orbe: dirección unitaria + anillo de cada punto.
function orbLattice(latRings, lonDensity) {
  const dirs = [], ring = [];
  for (let li = 0; li <= latRings; li++) {
    const lat = -Math.PI / 2 + (li / latRings) * Math.PI;
    const c = Math.cos(lat), s = Math.sin(lat);
    const lonCount = Math.max(1, Math.round(Math.abs(c) * lonDensity));
    for (let lj = 0; lj < lonCount; lj++) {
      const lon = (lj / lonCount) * 2 * Math.PI;
      dirs.push(c * Math.cos(lon), s, c * Math.sin(lon));
      ring.push(li);
    }
  }
  return { dirs: Float32Array.from(dirs), ring: Uint16Array.from(ring), count: ring.length, latRings };
}

// Asigna cada partícula a un punto (reparto parejo) con un desplazamiento
// UNIFORME dentro de una bola → punto de borde firme. La gaussiana original
// (2026-09-21 AM) daba nubes suaves que, con el bloom, se leían borrosas.
function genOrb(n, r, latRings, lonDensity, dotFrac) {
  const lat = orbLattice(latRings, lonDensity);
  ...
  // Radio del cúmulo proporcional al espaciado entre anillos.
  const clusterR = ((Math.PI * r) / latRings) * dotFrac;
  return {
    lat, owner, off, clusterR, r,
    pos: new Float32Array(n * 3),
    cx: new Float32Array(lat.count * 3),
  };
}

function writeOrb(o) {
  const { owner, off, cx, pos, clusterR: s } = o;
  for (let i = 0, n = owner.length; i < n; i++) {
    const d = owner[i], ix = i * 3, dx = d * 3;
    pos[ix]     = cx[dx]     + off[ix]     * s;
    pos[ix + 1] = cx[dx + 1] + off[ix + 1] * s;
    pos[ix + 2] = cx[dx + 2] + off[ix + 2] * s;
  }
}

// RUBIK: bandas que giran en cuartos de vuelta — revuelve y luego resuelve
// en reversa (palíndromo). Hoy es la INTRO del hero (ver RiskSphere). Las partículas se quedan en la
// retícula en reposo; el giro lo aplica el SHADER (uTwist, ver rubikTwist en
// GLOBE_VERTEX_SHADER) con rubikTwistAt(): como forma default del hero no
// puede costar un loop de CPU por frame sobre 160k-220k partículas.
export const RUBIK_MOVES = 14;
const rubikMoves = Array.from({ length: RUBIK_MOVES }, (_, i) => {
  const lo = -1 + 0.5 * Math.min(3, Math.floor(orbHash(i, 5.9) * 4));
  return {
    axis: Math.min(2, Math.floor(orbHash(i, 2.3) * 3)),
    lo, hi: lo + 0.5,
    ang: ((orbHash(i, 7.7) < 0.5 ? 1 : -1) * Math.PI) / 2,
  };
});
...
// Escribe en `out` (arreglo de RUBIK_MOVES Vector4) los giros del instante t
// escalados por `k` (0..1 — permite deshacer el cubo con suavidad al pasar
// al globo), usando los primeros `count` movimientos (la intro usa pocos).
// Devuelve el índice del movimiento activo (la banda que "engorda").
export function rubikTwistAt(t, out, k = 1, count = RUBIK_MOVES) {
  const sc = solveCycle(t, count, 0.42, 1.2);
  for (let i = 0; i < RUBIK_MOVES; i++) {
    const m = rubikMoves[i];
    out[i].set(m.axis, m.lo, m.hi, i < count ? m.ang * sc.amount[i] * k : 0);
  }
  return sc.active;
}

export function genRubik(n, r) {
  // Retícula densa (~2.5k puntos, 30-90 partículas c/u según el tier) y
  // puntos chicos: más puntos nítidos en vez de pocas nubes grandes.
  const o = genOrb(n, r, 40, 100, 0.12);
  const { dirs, count } = o.lat;
  for (let d = 0; d < count * 3; d++) o.cx[d] = dirs[d] * r;
  writeOrb(o);
  return o;
}
...
export const HERO_FORMS = [
  ...
  {
    id: "RUBIK",
    label_es: "Rotación Sectorial", label_en: "Sector Rotation",
    formula: "Σ wᵢ = 1",
    es: "Una banda gira y todo el portafolio se reacomoda: el capital no desaparece, cambia de sector. Cada rotación parece caos; vista completa, es el mismo cubo reordenándose — y tarde o temprano vuelve a cuadrar.",
    en: "One band turns and the whole portfolio rearranges: capital doesn't vanish, it changes sector. Each rotation looks like chaos; seen whole, it's the same cube reordering itself — and sooner or later it squares up again.",
  },
];
