...
// coordenadas unitarias de la esfera (pos / uTwistR). uTwistOn = 0 → sin
// giro, cero costo. uCube: 1 = esfera Rubik (cuadrícula, océano más claro),
// 0 = globo normal.
...
void main() {
  ...
  vec3 pos = position;
  // Cuadrícula de la esfera Rubik: los cortes entre capas son los planos
  // x, y, z = ±1/3 (en la esfera unitaria dan zonas de igual área —
  // Arquímedes) y se ven como círculos que la parten en las 26 piezas de un
  // 3×3. Se calcula con la posición ANTES del giro: la línea viaja con su
  // pieza, y un cuarto de vuelta lleva la cuadrícula sobre sí misma.
  vGrid = 0.0;
  if (uCube > 0.001) {
    vec3 d3 = abs(abs(position / uTwistR) - 1.0 / 3.0);
    vGrid = (1.0 - smoothstep(0.006, 0.022, min(d3.x, min(d3.y, d3.z)))) * uCube;
  }
...
// ── Esfera Rubik: intro del hero (2026-09-21) ────────────────────────────
// El MISMO globo partido como un cubo 3×3 (capas cortadas por los planos
// ±1/3): arranca revuelto y se resuelve; resuelta ES el globo, así que el
// paso a la vista normal no mueve una sola partícula. Idea del orbe
// "solving" de thinking-orbs (MIT © Jakub Antalik, RareFormLabs).

// Revoltura 3×3 fija (mismo arranque en cada visita): la esfera ARRANCA con
...
// coordenadas unitarias (polos en ±1, con margen para no dejar fuera las
// partículas que caen justo ahí).
...
export const HERO_FORMS = [
  ...
  {
    id: "CUBE",
    label_es: "Esfera Rubik", label_en: "Rubik sphere",
    formula: "",
    es: "Intro del hero: el globo partido como un cubo 3×3, revuelto, que se resuelve hasta completar el mapa.",
    en: "Hero intro: the globe split like a 3×3 cube, scrambled, solving itself until the map is whole.",
  },
];
