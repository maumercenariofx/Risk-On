...
uniform float uScan;
// Giro Rubik de la intro: hasta 8 giros de capa. Cada vec4 es (eje 0/1/2,
// límite inferior, límite superior, ángulo YA escalado por su avance), en
// coordenadas unitarias del cubo (pos / uTwistR, caras en ±1). uTwistOn = 0
// → sin giro, cero costo. uCube: 1 = cubo (cuadrícula 3×3, normal de cara),
// 0 = esfera.
uniform vec4  uTwist[8];
...
void main() {
  ...
  vec3 pos = position;
  // Cuadrícula 3×3 de cada cara: se calcula con la posición ANTES del giro,
  // así las líneas viajan con su pieza — un cuarto de vuelta lleva la
  // cuadrícula sobre sí misma.
  vGrid = 0.0;
  if (uCube > 0.001) {
    vec3 q = position / uTwistR;
    vec3 aq = abs(q);
    vec2 f = (aq.x >= aq.y && aq.x >= aq.z) ? q.yz : ((aq.y >= aq.z) ? q.xz : q.xy);
    vec2 d = min(abs(f - 1.0 / 3.0), abs(f + 1.0 / 3.0));
    d = min(d, 1.0 - abs(f));
    vGrid = (1.0 - smoothstep(0.012, 0.045, min(d.x, d.y))) * uCube;
  }
  // Giro Rubik de la intro (ver uTwist).
  ...
  // En el cubo cuenta la normal de su CARA, no la radial: si no, el corte
  // del lado lejano se come las orillas de las caras visibles.
  vec3 np = normalize(pos);
  if (uCube > 0.001) {
    vec3 ap = abs(pos);
    vec3 fn = (ap.x >= ap.y && ap.x >= ap.z) ? vec3(sign(pos.x), 0.0, 0.0)
            : ((ap.y >= ap.z) ? vec3(0.0, sign(pos.y), 0.0) : vec3(0.0, 0.0, sign(pos.z)));
    np = normalize(mix(np, fn, uCube));
  }
  vec3 viewNormal = normalize(normalMatrix * np);
  ...
  float size = uSize * (1.0 + vBorder * uColorT * 0.9) * (1.0 + vScan * 0.9);
...
void main() {
  ...
  // En el cubo el océano sube: una cara casi toda de mar se perdía en negro
  // y el cubo no se leía como cubo.
  oceanColor *= 1.0 + 1.6 * uCube;
  ...
  // Cuadrícula del cubo: las juntas entre piezas apagan sus puntos.
  finalColor *= 1.0 - 0.9 * vGrid;
...
// ── Cubo del mapa: intro del hero (2026-09-21) ───────────────────────────
// Las MISMAS partículas del globo proyectadas desde el centro sobre un cubo
// (gnomónica): cada cara muestra su pedazo del mapa porque el shader lo
// muestrea con normalize(position), idéntico en cubo y en esfera. Por eso
// inflar el cubo a esfera es un morph radial sin costuras. Idea del orbe
// "solving" de thinking-orbs (MIT © Jakub Antalik, RareFormLabs).
export const CUBE_H = 0.58; // semilado del cubo, en unidades de R (cabe completo; al inflarse "crece" a globo)
export function genCube(n, r) {
  const pos = genSphere(n, r);
  for (let i = 0; i < n; i++) {
    const ix = i * 3, x = pos[ix], y = pos[ix + 1], z = pos[ix + 2];
    const k = (r * CUBE_H) / Math.max(Math.abs(x), Math.abs(y), Math.abs(z));
    pos[ix] = x * k; pos[ix + 1] = y * k; pos[ix + 2] = z * k;
  }
  return pos;
}

// Revoltura 3×3 fija (mismo arranque en cada visita): el cubo ARRANCA con
// todos los giros aplicados y se resuelve deshaciéndolos en orden inverso,
// así cada giro "completa" un poco más el mapa. Ejes alternados; capas en
// coordenadas unitarias (caras en ±1, con margen para no dejar fuera las
// partículas que caen justo en la cara).
export const TWIST_SLOTS = 8; // tamaño del arreglo uTwist (shader y JS)
export const CUBE_MOVES = 6;
const LAYER = [[-1.01, -1 / 3], [-1 / 3, 1 / 3], [1 / 3, 1.01]];
const cubeMoves = [
  { axis: 1, layer: 2, dir: 1 },
  { axis: 0, layer: 0, dir: -1 },
  { axis: 2, layer: 2, dir: 1 },
  { axis: 1, layer: 0, dir: -1 },
  { axis: 0, layer: 2, dir: 1 },
  { axis: 1, layer: 1, dir: 1 },
].map((m) => ({ axis: m.axis, lo: LAYER[m.layer][0], hi: LAYER[m.layer][1], ang: (m.dir * Math.PI) / 2 }));

// Estado del cubo a los `t` segundos de solución (`slot` s por giro), con los
// ángulos escalados por `k`. Escribe en `out` (TWIST_SLOTS Vector4).
export function cubeSolveAt(t, out, slot, k = 1) {
  const done = Math.floor(t / slot);          // giros ya deshechos
  const p = (t - done * slot) / slot;
  const ep = 1 - (1 - Math.min(1, p / 0.7)) ** 3; // easing "de máquina"
  const undo = CUBE_MOVES - 1 - done;         // el que se está deshaciendo
  for (let i = 0; i < out.length; i++) {
    if (i >= CUBE_MOVES) { out[i].set(0, 0, 0, 0); continue; }
    const m = cubeMoves[i];
    const a = i > undo ? 0 : i === undo ? 1 - ep : 1;
    out[i].set(m.axis, m.lo, m.hi, m.ang * a * k);
  }
}
...
export const HERO_FORMS = [
  ...
  {
    id: "CUBE",
    label_es: "Cubo del mapa", label_en: "Map cube",
    formula: "",
    es: "Intro del hero: el mapa revuelto en un cubo que se resuelve y se infla hasta volverse el globo.",
    en: "Hero intro: the map scrambled on a cube that solves itself and inflates into the globe.",
  },
];
